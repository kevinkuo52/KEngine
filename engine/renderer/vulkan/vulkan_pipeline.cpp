...
VulkanPipeline::VulkanPipeline(Window& window, MainCamera& mainCamera, VulkanDevice& device, Scene& scene) : 
    window(window), mainCamera(mainCamera), device(device), scene(scene)
{
    InitVulkan();
}
...
void VulkanPipeline::CreateGraphicsPipeline() {
    auto vertShaderCode = ReadFile("engine/renderer/shader/vert.spv");
    auto fragShaderCode = ReadFile("engine/renderer/shader/frag.spv");
...
void VulkanPipeline::TransitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout) {
...
VkImageView VulkanPipeline::CreateImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags) {
